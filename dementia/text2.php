...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if (mysqli_query($link, $sql_plan)) {
        echo "<script>alert('新增成功'); window.location.href = 'text2.php';</script>";
    } else {
    ...
    <script>
        ...
        function generateCalendar(year, month, selectedDate) {
            ...
            for (let day = 1; day <= daysInCurrentMonth; day++) {
                ...
                // 高亮當前日期或者選中的日期
                if ((year === today.getFullYear() && month === today.getMonth() && day === today.getDate()) || (year === selectedDate.year && month === selectedDate.month && day === selectedDate.day)) {
...
// 存儲選中的日期到 localStorage
        ...
        window.addEventListener('load', function () {
            ...
            if (selectedDateFromStorage) {
                ...
                // 將日曆更新到該選中的日期
                const [year, month, day] = selectedDateFromStorage.split('-');
                currentYear = parseInt(year);
                currentMonth = parseInt(month) - 1;  // month 是 0-11，所以要減 1
        ...
        document.getElementById('saveChangesBtn').addEventListener('click', function () {
            ...
            xhr.open("POST", "text2.php", true);

            // 使用 onload 來處理請求完成後的回應
            xhr.onload = function () {
                if (xhr.status === 200) {
                    if (xhr.responseText.trim() === 'success') {
                        ...
                        alert('錯誤: ' + xhr.responseText);  // 顯示錯誤訊息
                    }
                } else {
                    alert('錯誤: ' + xhr.statusText);  // 顯示狀態碼錯誤訊息
                }
            };
            // 發送請求
            xhr.send(formData);
