...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    if (mysqli_query($link, $sql_delete)) {
        echo "
<script>alert('活動刪除成功'); window.location.href = 'activity_management.php';</script>";
    } else {
        echo "
<script>alert('刪除失敗: " . mysqli_error($link) . "');</script>";
    }
    exit();
}
...
<body>
                        ...
                        <div class="modal-footer">
                            <div class="d-flex justify-content-between w-100">
                                <button type="button" class="btn btn-primary  add-button" id="addQuestionButton"
                                    onclick="addQuestion()">新增問題</button>
                                <div>
    ...
    <script>
        ...
        document.getElementById('editForm').addEventListener('submit', function (e) {
            ...
            const formData = new FormData(this); // 使用 FormData 收集表单数据
            formData.append('action', 'edit'); // 添加操作类型
            ...
            fetch('', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        setTimeout(() => {
                            alert('活動更新成功');
                            window.location.reload(); // 刷新页面以查看更新
                        }, 1000); // 500 毫秒的延遲時間
                    } else {
        ...
        function addQuestion() {
            ...
            <div id="optionsContainer${questionIndex}" style="display: none;">
                ...
                <button type="button" class="btn btn-secondary" onclick="addOption(${questionIndex})"
                    style="display: none;">
