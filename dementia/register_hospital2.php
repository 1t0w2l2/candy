    ...
    if (!empty($institution_name) && !empty($institution_address) && !empty($city) && !empty($town) && !empty($institution_latlng) && !empty($institution_phone) && !empty($institution_id) && !empty($_POST['business_hours'])) {
        ...
        if (empty($hospital_row)) {
            ...
            $sql_insert = "INSERT INTO `institution` (`institution_id`, `institution_name`, `county`, `town`, `address`, `phone`, `person_charge`, `website`, `lat`, `lng`) 
                           VALUES ('$institution_id', '$institution_name', '$city', '$town', '$institution_address', '$institution_phone', '$person_charge', '$institution_url', '$latitude', '$longitude')";
            ...
            // 更新現有的機構資料
            $sql = "UPDATE `institution` SET `institution_name`='$institution_name',`county`='$city',`town`='$town',`address`='$institution_address',`phone`='$institution_phone',`person_charge`='$person_charge',`website`='$institution_url',`lat`='$latitude',`lng`='$longitude' WHERE `institution_id`='$institution_id'";
        ...
        if (isset($_POST['business_hours'])) {
            ...
            foreach ($business_hours as $day => $times) {
        ...
        // 處理 hospital 表的插入
        $sql = "INSERT INTO `hospital`(`account`, `institution_id`, `institution_name`, `institution_address`, `institution_phone`) 
                VALUES ('$account','$institution_id','$institution_name','$institution_address','$institution_phone')";
        ...
        if (move_uploaded_file($institution_img['tmp_name'], $target_file)) {
            ...
            echo "<script>alert('感谢您！已成功送出帳號申請，請到信箱查收驗證碼！'); window.location.href = 'email.php';</script>";
            exit;
        } else {
